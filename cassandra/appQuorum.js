...
if (cluster.isMaster) {
    ...
    app.get("/v1/readUser/:id", (req, res, next) => {
        const query = 'SELECT * FROM test_keyspace.user_quorum_rw WHERE id = ?';
        let id = req.params.id;

        // Set the prepare flag in your queryOptions
        client.execute(query, [id], { prepare: true , consistency: types.consistencies.quorum}, function (err, results) {
            if (err) {
                console.log(err)
                throw err
            }
    ...
    app.post('/v1/addUser', function (req, res) {
        let data = req.body
        const query = 'INSERT INTO test_keyspace.user_quorum_rw(id,channel,countrycode,createdby,createddate,currentlogintime,dob,email,emailverified,firstname,flagsvalue,framework,gender,grade,language,lastname,loginid,password,phone,phoneverified,profilevisibility,status,updatedby,updateddate,userid,username,usertype) values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)';
        ...
        client.execute(query, params, { prepare: true, consistency: types.consistencies.quorum }, function (err) {
            if (err) {
                console.log(err)
                throw err
            }
...
const params = [data.id, "testchannel", "+91", data.createdby, data.createddate, data.currentlogintime,
        ...
        client.execute(query, params, {prepare: true, consistency: types.consistencies.quorum }, function (err) {
            ...
            const readQuery = 'SELECT * FROM test_keyspace.user_quorum_rw WHERE id = ?';
            client.execute(readQuery, [data.id], { prepare: true, consistency: cassandra.types.consistencies.quorum }, function (err, results) {
                if (err) {
                    console.log(err)
                    throw err
                }
